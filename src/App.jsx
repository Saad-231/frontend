...
export default function App() {
  const { limitModal, setLimitModal, sidebarOpen, setSidebarOpen, theme } = useAppContext();
  ...
  useEffect(() => {
    if (!localStorage.getItem('novascribe_onboarded')) {
  ...
  if (isLoading) {
    return (
      <div className="app-loading">
        ...
        <div className="app-loading__text">NovaScribe.AI</div>
      </div>
    );
  }
  ...
  return (
    <div className="app-shell app-shell--entering">
      <Sidebar />
      {sidebarOpen && (
        <div className="app-shell__mobile-backdrop" onClick={() => setSidebarOpen(false)} aria-hidden="true" />
      )}

      <ChatArea />

      {limitModal && <LimitModal payload={limitModal} onClose={() => setLimitModal(null)} />}
